...
smalltalk.prompt('身份验证', '你的名字?', '').then(function(name) {
	...
	socket.on('login', function (user) {
		if(user.length>=1){
			for(var i =0;i<user.length;i++){
				incomeHtml(user[i],'src/img/gege4.png');
			}
		}
	});
	// 监听中途的成员加入
	socket.on('user joined', function (tname, index) {
		incomeHtml(tname,'src/img/gege3.png');
		console.log(tname+'加入');
		showNotice('src/img/gege4.png',tname,"上线了");
	});
	// 接收私聊信息
	socket.on('receive private message', function (data) {
		$('#ding')[0].play();
		if(data.addresser == data.recipient) return;
		var head = 'src/img/head.png';
	...
	$(document).on('click','.chat-active .send',function(){
		...
		if(val == '') return;
		sendMessage('src/img/head.png',val);
		//call
		var req = {
			'addresser':name,
			'recipient':recipient,
